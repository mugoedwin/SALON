...
import { fetchSalonServices } from "../services/salonServicesApi";
...
function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    fetchSalonServices()
      .then((services) => {
        if (isMounted) {
          setLiveServiceCount(Array.isArray(services) ? services.length : 0);
        }
      })
      .catch((error) => {
  ...
  const dashboardStats = useMemo(
    () => [
      { label: "Live Services", value: liveServiceCount.toString() },
      { label: "Pending Bookings", value: pendingCount.toString() },
      { label: "WhatsApp Leads", value: "-" },
    ],
    [liveServiceCount, pendingCount],
  );
