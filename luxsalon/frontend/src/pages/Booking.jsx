    ...
    >
      <div className="grid gap-4 sm:grid-cols-[160px_minmax(0,1fr)_auto] sm:items-start">
        ...
        <div className="min-w-0">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-rose-700">
            {service.bookingSource || "Live service"}
          </p>
          <div className="mt-2 flex flex-wrap items-baseline gap-3">
            <h3 className="text-2xl font-semibold text-salon-strong">{service.title}</h3>
          ...
          <p className="mt-3 text-sm leading-7 text-salon-muted">
            {service.bookingSource || ""}
          </p>
        </div>
...
function Booking() {
  ...
  const selectedService = useMemo(
    () =>
      liveServices.find((service) => service.title === selectedServiceName) ??
      null,
    [selectedServiceName, liveServices],
  );
        ...
        <div className="space-y-5">
          {liveServices.map((service) => (
            <ServiceOption
              ...
              service={service}
              isSelected={service.title === selectedServiceName}
              onSelect={() => setSelectedServiceName(service.title)}
            />
          ))}
        </div>
      );
        ...
        <aside className="space-y-6 self-start lg:sticky lg:top-28">
          <div className="border border-rose-100 bg-rose-50/70 px-5 py-5 sm:px-6">
            ...
            <dl className="mt-4">
              ...
              <SummaryRow
                label="Price"
                value={selectedService?.priceLabel ?? "AED --"}
              />
              <SummaryRow
                label="Duration"
